...
export function CalendarioFlotante() {
  ...
  const colorPunto = (n) => {
    if (n <= 0) return null
    if (n <= umbrales.verde)    return '#16a34a'
    if (n <= umbrales.amarillo) return '#d97706'
    return '#dc2626'
  }
  ...
  return (
    <div style={{ padding:'12px' }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'10px' }}>
        ...
        <span style={{ fontFamily:'var(--font-display)', fontWeight:'600', fontSize:'14px' }}>{MESES[mes]} {anio}</span>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(7,1fr)', gap:'2px', marginBottom:'4px' }}>
        {DIAS.map((d,i) => <div key={i} style={{ textAlign:'center', fontSize:'11px', fontWeight:'500', color:'var(--muted)', padding:'4px 0' }}>{d}</div>)}
      </div>
            ...
            <div key={i} onClick={() => { if (!d) return; setSel({ d, m: mes, a: anio }); setPanelAbierto(true) }}
              ...
              {d && col && (
                <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:'2px', marginTop:'1px' }}>
                  ...
                  {n > 1 && <div style={{ fontSize:'9px', fontWeight:'600', color: isSel(d) ? 'white' : col, lineHeight:1 }}>{n}</div>}
                </div>
              )}
            </div>
          )
      ...
      {loadingActs && <div style={{ textAlign:'center', fontSize:'11px', color:'var(--muted)', marginTop:'8px' }}>Cargando...</div>}
        ...
        <>
          ...
          <div style={{
            ...
            <div style={{ padding:'8px 20px 14px', borderBottom:'1px solid var(--border)', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
              <div>
                <div style={{ fontFamily:'var(--font-display)', fontWeight:'600', fontSize:'17px' }}>
